(() => {
  ...
  function ensureVisualStage(){
    ...
    if(!$('v120-play-stage')){
      ...
      stage.setAttribute('aria-label','本次出牌视觉反馈');
      table.appendChild(stage);
    }
    if(!$('v120-pass-flash')){
      const bubble=document.createElement('div');
      bubble.id='v120-pass-flash';
      bubble.className='v120-pass-flash';
  ...
  function ensureOrientationNotice(){
    ...
    notice.id='v120-orientation-notice';
    notice.className='v120-orientation-notice';
    notice.innerHTML='<div class="v120-orientation-card"><b>横屏牌桌 · 可视化优先</b><p>手机/平板使用横屏可获得更大的扑克牌与更清晰的出牌反馈。点击下方按钮尝试进入横屏牌桌；若浏览器不允许自动旋转，请将设备旋转为横屏。</p><button id="v120-landscape-now" type="button">进入横屏牌桌</button></div>';
    document.body.appendChild(notice);
    $('v120-landscape-now').addEventListener('click',requestLandscape);
  }
  ...
  function showPlay(play){
    ...
    stage.classList.remove('show','is-bomb');
  ...
  function showPass(player){
    ensureVisualStage();
    const bubble=$('v120-pass-flash');
    if(!bubble) return;
    bubble.className=`v120-pass-flash ${player===0?'me':player===1?'left':'right'}`;
    bubble.textContent=`${playerName(player)} · 不要`;
    void bubble.offsetWidth;
    bubble.classList.add('show');
    setTimeout(()=>bubble.classList.remove('show'),920);
  }

  function ensureAutoplayControl(){
    const old=$('hint');
    if(!old) return null;
    if(old.dataset.qilyAutoplay==='1') return old;
    const button=old.cloneNode(true);
    button.id='hint';
    button.dataset.qilyAutoplay='1';
    button.classList.add('autoplay-btn');
    old.replaceWith(button);
    button.addEventListener('click',()=>setAutoplay(!autoplay,true));
    return button;
  }

  function ensureSettingsToggle(){
    const grid=document.querySelector('.settings-grid');
    if(!grid||$('setting-qily-autoplay')) return;
    const row=document.createElement('label');
    row.className='setting-row';
    row.innerHTML='<span><b>启力托管</b><small>默认关闭，关闭时完全自主判断；开启后由启力 AI 自动叫分和出牌。</small></span><input id="setting-qily-autoplay" type="checkbox">';
    const difficulty=grid.querySelector('.setting-select:last-of-type');
    grid.insertBefore(row,difficulty||null);
    $('setting-qily-autoplay').addEventListener('change',event=>setAutoplay(event.target.checked,true));
  }

  function setAutoplay(next,announce=false){
    autoplay=Boolean(next);
    ...
    if(announce){
      const toast=$('toast');
      if(toast){
        toast.textContent=autoplay?'启力托管已开启：自动叫分与出牌':'启力托管已关闭：现在由你自主判断出牌';
  ...
  function updateAutoplayUi(){
    const button=ensureAutoplayControl();
    if(button){
      button.textContent=`启力托管：${autoplay?'开':'关'}`;
      button.classList.toggle('on',autoplay);
      button.setAttribute('aria-pressed',String(autoplay));
      button.title=autoplay?'点击关闭托管，恢复自主出牌':'点击开启托管，由启力 AI 自动出牌';
    }
    const setting=$('setting-qily-autoplay');
    if(setting) setting.checked=autoplay;
    const play=$('play');
    if(play) play.textContent='出牌';
  }
  ...
  function scheduleAutoplay(state){
    ...
    autoplayTimer=setTimeout(async()=>{
      autoplayTimer=null;
      if(!autoplay||!window.PureDDZTest) return;
      const fresh=window.PureDDZTest.getState();
      if(currentTurnToken(fresh)!==token||fresh.current!==0) return;
      try{
        ...
        if(!choice?.cards?.length){
          const pass=$('pass');
          if(pass&&!pass.disabled) pass.click(); else managedTurnToken='';
          return;
        }
        choice.cards.forEach(card=>{
          const button=document.querySelector(`#hand .card[data-id="${card.id}"]`);
          if(button&&!button.classList.contains('selected')) button.click();
        });
        await sleep(80);
        const play=$('play');
        if(play&&!play.disabled) play.click(); else managedTurnToken='';
      }catch(_error){ managedTurnToken=''; }
    },620);
  }
  ...
  function refresh(){
    ...
    ensureVisualStage(); ensureSettingsToggle(); updateAutoplayUi(); updateOrientationNotice();
    const state=window.PureDDZTest.getState();
    detectPass(previousState,state);
    const signature=playSignature(state.lastPlay);
    if(signature) showPlay(state.lastPlay);
    else if(state.phase!=='bidding') clearPlayStage();
    scheduleAutoplay(state);
    previousState=state;
  }

  function start(){
    ensureVisualStage(); ensureOrientationNotice(); ensureSettingsToggle(); updateAutoplayUi(); updateOrientationNotice();
    window.addEventListener('orientationchange',()=>setTimeout(updateOrientationNotice,160));
    window.addEventListener('resize',updateOrientationNotice);
    ['welcome-start','start','again'].forEach(id=>$(id)?.addEventListener('click',()=>{ if(isTouchMobile()) requestLandscape(); },{passive:true}));
    document.addEventListener('pointerdown',()=>{ if(isTouchMobile()&&window.matchMedia('(orientation:portrait)').matches) requestLandscape(); },{once:true,passive:true});
